...
def pair(left,right,focus='both',labels=('Without WIC','With WIC')):
 return f'<div class="pair focus-{focus}"><div class="path left"><span class="path-label">{labels[0]}</span><div class="world">{left}</div></div><div class="path right"><span class="path-label">{labels[1]}</span><div class="world">{right}</div></div></div>'
...
for i,s in enumerate(scenes):
 ...
 for j,b in enumerate(s['beats']):
  source=f' <a href="#source-{b["source"]}" class="source-link">See details</a>' if b['source'] else ''
  focus='research' if 'class="research"' in b['visual'] else 'ending' if 'ending-art' in b['visual'] else 'left' if b['who'].startswith('Without WIC') else 'right' if b['who'].startswith('With WIC') else 'both'
...
page='''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex, nofollow, noarchive"><meta name="googlebot" content="noindex, nofollow, noarchive"><meta name="theme-color" content="#f8f6f0"><title>The work of getting help — WIC</title><meta name="description" content="Follow one mother down two paths to see what WIC changes—and how much work it takes to keep getting it."><link rel="stylesheet" href="story.css?v=20260925-focus5"><script type="module" src="story.js?v=20260925-focus5"></script></head><body><main><section class="opening" aria-label="Available does not equal accessible"><div class="opening-pin"><div class="equation"><span class="available">Available</span><span class="symbol">≠</span><span class="accessible">Accessible</span></div><p class="scroll-cue">A story about the distance between the two<br><span>Scroll to begin ↓</span></p></div></section><section class="introduction"><p class="eyebrow">Available ≠ Accessible · WIC</p><h1>The work<br>of getting help</h1><p class="dek">WIC helps families afford nutritious food. Follow one mother down two paths to see what the help changes—and how much work it takes to keep getting it.</p><p class="byline">By Megan Quigley</p><p class="intro-tail">One mother. Two paths. Five years.</p></section>'''+''.join(parts)+end+'<dialog id="source-dialog" aria-labelledby="detail-title"><button class="close-details" aria-label="Close details">Close ×</button><div class="detail-content"></div></dialog><script id="basket-data" type="application/json">'+json.dumps(BASKET['items'])+'</script></main></body></html>'
